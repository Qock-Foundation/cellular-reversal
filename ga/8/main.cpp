...
class creature {  // 3D CA
  ...
  static std::pair<int, int> rotated_substrings_heuristics(const uint8_t arr[H][N][M], const uint8_t str[kCAIOlen]) {
    ...
    for (int len = 3; len <= kCAIOlen; ++len) {
      ...
      //score += 3 * (len == 3 ? 2 : len == 4 ? 4 : len == 5 ? 6 : len == 6 ? 8 : len == 7 ? 10 : 12) * diagonal_easy[len] * (diagonal_easy[len] <= H * kCAIOlen / len);
      //score -= 3 * (len == 3 ? 1 : len == 4 ? 1 : len == 5 ? 1 : len == 6 ? 1 : len == 7 ? 1 : 1) * diagonal_easy_fake[len];
