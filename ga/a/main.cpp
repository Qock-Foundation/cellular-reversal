...
const int kCACntCellTypes = 7, kCACntStrLetterTypes = 2;
const int N = 21, M = 19, H = 55, V = H * N * M, A = N * M;
const int kCAIOi0 = 4, kCAIOj0 = 5, kCAIOlen = 9;

std::atomic<int64_t> aTotalDiffScore, aTotalVolumeScore, aTotalBorderScore, aTotalSubstringsScore, aTotalTrashScore, aTotalAreaDiffScore, aTotalFiberizationScore, aTotalGenesScore, aTotalLastAreaScore, aTotalLettersScore;
...
class creature {  // 3D CA
  ...
  static std::pair<int, int> rotated_substrings_heuristics(const uint8_t arr[H][N][M], const uint8_t str[kCAIOlen]) {
    ...
    int direct[kCAIOlen+1] = {}, diagonal_easy[kCAIOlen+1] = {}, perpendicular[kCAIOlen+1] = {}, diagonal_hard[kCAIOlen+1] = {}, reversed[kCAIOlen+1] = {},
        direct_fake[kCAIOlen+1] = {}, diagonal_easy_fake[kCAIOlen+1] = {}, perpendicular_fake[kCAIOlen+1] = {}, diagonal_hard_fake[kCAIOlen+1] = {}, reversed_fake[kCAIOlen+1] = {};
    for (int t = 0; t < H; ++t) {
      for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
          ...
          for (int offset = 0; offset < 1; ++offset) {
            int u;
            for (u = 0; j + u < M && offset + u < kCAIOlen && arr[t][i][j+u] == str[offset+u]; ++u);  // matching rightwards
            direct[u]++;
            for (; j + u < M && offset + u < kCAIOlen && 1 <= arr[t][i][j+u] && arr[t][i][j+u] <= kCACntStrLetterTypes; ++u);
            direct_fake[u]++;
            for (u = 0; i + u < N && j - u >= 0 && offset + u < kCAIOlen && arr[t][i+u][j-u] == str[offset+u]; ++u);  // matching right-up-wards
            diagonal_easy[u]++;
            for (; i + u < N && j - u >= 0 && offset + u < kCAIOlen && 1 <= arr[t][i+u][j-u] && arr[t][i+u][j-u] <= kCACntStrLetterTypes; ++u);
            diagonal_easy_fake[u]++;
            for (u = 0; i + u < N && j + u < N && offset + u < kCAIOlen && arr[t][i+u][j+u] == str[offset+u]; ++u);  // matching right-down-wards
            diagonal_easy[u]++;
            for (; i + u < N && j + u < N && offset + u < kCAIOlen && 1 <= arr[t][i+u][j+u] && arr[t][i+u][j+u] <= kCACntStrLetterTypes; ++u);
            diagonal_easy_fake[u]++;
            /*for (u = 0; i - u >= 0 && offset + u < kCAIOlen && arr[t][i-u][j] == str[offset+u]; ++u);  // matching upwards
            perpendicular[u]++;
            for (; i - u >= 0 && offset + u < kCAIOlen && 1 <= arr[t][i-u][j] && arr[t][i-u][j] <= kCACntStrLetterTypes; ++u);
            perpendicular_fake[u]++;
            for (u = 0; i + u < N && offset + u < kCAIOlen && arr[t][i+u][j] == str[offset+u]; ++u);  // matching downwards
            perpendicular[u]++;
            for (; i + u < N && offset + u < kCAIOlen && 1 <= arr[t][i+u][j] && arr[t][i+u][j] <= kCACntStrLetterTypes; ++u);
            perpendicular_fake[u]++;
            for (u = 0; i - u >= 0 && j - u >= 0 && offset + u < kCAIOlen && arr[t][i-u][j-u] == str[offset+u]; ++u);  // matching left-up-wards
            diagonal_hard[u]++;
            for (; i - u >= 0 && j - u >= 0 && offset + u < kCAIOlen && 1 <= arr[t][i-u][j-u] && arr[t][i-u][j-u] <= kCACntStrLetterTypes; ++u);
            diagonal_hard_fake[u]++;
            for (u = 0; i - u >= 0 && j + u < N && offset + u < kCAIOlen && arr[t][i-u][j+u] == str[offset+u]; ++u);  // matching left-down-wards
            diagonal_hard[u]++;
            for (; i - u >= 0 && j + u < N && offset + u < kCAIOlen && 1 <= arr[t][i-u][j+u] && arr[t][i-u][j+u] <= kCACntStrLetterTypes; ++u);
            diagonal_hard_fake[u]++;
            for (u = 0; j - u >= 0 && offset + u < kCAIOlen && arr[t][i][j-u] == str[offset+u]; ++u);  // matching leftwards!
            reversed[u]++;*/
          }
        }
      }
    ...
    for (int len = 3; len <= kCAIOlen; ++len) {
      //score += (len == 3 ? 2 : len == 4 ? 4 : len == 5 ? 6 : len == 6 ? 8 : len == 7 ? 16 : 32) * direct[len] * (direct[len] <= H * kCAIOlen / len / 2); 
      //score -= (len == 3 ? 1 : len == 4 ? 1 : len == 5 ? 1 : len == 6 ? 1 : len == 7 ? 1 : 1) * direct_fake[len];
      score += 2 * (len == 3 ? 2 : len == 4 ? 4 : len == 5 ? 6 : len == 6 ? 8 : len == 7 ? 10 : 12) * diagonal_easy[len] * (diagonal_easy[len] <= H * kCAIOlen / len);
      score -= 1 * (len == 3 ? 1 : len == 4 ? 1 : len == 5 ? 1 : len == 6 ? 1 : len == 7 ? 1 : 1) * diagonal_easy_fake[len];
    ...
    for (int t = 0; t < H; ++t) {
      for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
          if (arr[t][i][j] == 0 or arr[t][i][j] > kCACntStrLetterTypes) {
            continue;
          }
          penalty += i < N - 1 && 1 <= arr[t][i+1][j] && arr[t][i+1][j] <= kCACntStrLetterTypes;
          penalty += i < N - 1 && j < M - 1 && 1 <= arr[t][i+1][j+1] && arr[t][i+1][j+1] <= kCACntStrLetterTypes;
          penalty += j < M - 1 && 1 <= arr[t][i][j+1] && arr[t][i][j+1] <= kCACntStrLetterTypes;
          penalty += i > 0 && j < M - 1 && 1 <= arr[t][i-1][j+1] && arr[t][i-1][j+1] <= kCACntStrLetterTypes;
        }
      }
    }
...
public:
  ...
  creature sex(const creature& another, std::mt19937& generator) {  // another is const
    ...
    // and mutations
    static const double kCAFracMutations = 0.03;
    for (int i = 0; i < kCACntGenes; ++i) {
  ...
  int64_t score(int trials, std::mt19937& generator) {  // samples situations, estimates the creature
    ...
    for (int situation = 0; situation < trials; ++situation) {
      ...
      for (int i = 0; i < kCAIOlen; ++i) {
        s[i] = 1 + generator() % kCACntStrLetterTypes;
      }
      for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
          F[i][j] = 0;
      ...
      score -= ((double)cnt_nonzero_last_layer / A - kCAFracLastArea) * kCAFracLastArea
          ...
          * kCALastAreaLoss;*/
      {
        static const double kCARotatedSubstringsBonus = 2000;
      ...
      {
        static const double kCALettersLoss = 300;
        double cnt[kCACntCellTypes] = {};
        for (int t = 0; t < H; ++t) {
          for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
              cnt[spacetime[t][i][j]]++;
        ...
        double avg = 0;
        for (int i = 0; i < kCACntCellTypes; ++i) {
          cnt[i] = cnt[i] > 0 ? log(cnt[i] / double(cnt_nonzero_cells)) : -100;
          avg += cnt[i];
        }
        avg /= kCACntCellTypes;
        double var = 0;
        ...
        var /= kCACntCellTypes;
        score -= var * kCALettersLoss;
        aTotalLettersScore -= var * kCALettersLoss;
      }
      {
...
int main() {
  ...
  for (int T = 0;; ++T) {
    ...
    aTotalDiffScore = 0, aTotalVolumeScore = 0, aTotalBorderScore = 0, aTotalSubstringsScore = 0, aTotalTrashScore = 0,
                    aTotalAreaDiffScore = 0, aTotalFiberizationScore = 0, aTotalGenesScore = 0, aTotalLastAreaScore = 0, aTotalLettersScore = 0;
    ...
    std::cout << "Avg Letters Score (per assessment):\t\t" << aTotalLettersScore / cnt_assessments << "\n";
