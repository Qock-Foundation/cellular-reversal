...
const int kCAIOi0 = 4, kCAIOj0 = 3, kCAIOlen = 14;
...
public:
  ...
  field run(const field F, uint8_t arr[H][N][M], bool show_spacetime = false) {  // field, cnt nonzero cells in the run, cnt border cells in the run
    for (int i = 0; i < N; ++i) {
      for (int j = 0; j < M; ++j) {
        arr[0][i][j] = F[i][j];
      }
    ...
    if (rand() % 100000 == 0 || show_spacetime) {
  ...
  creature sex(const creature& another, std::mt19937& generator) {  // another is const
    ...
    // and mutations
    static const double kCAFracMutations = 0.1;
    for (int i = 0; i < kCACntGenes; ++i) {
  ...
  int64_t score(int trials, std::mt19937& generator) {  // samples situations, estimates the creature
    ...
    for (int situation = 0; situation < trials; ++situation) {
      ...
      F[kCAIOi0][kCAIOj0-1] = 4;
      uint8_t spacetime[H][N][M];
      const field G = run(F, spacetime);
      {
        for (int t = 0; t < H; ++t) {
          for (int i = 0; i < N - kCAIOlen + 1; ++i) {
            for (int j = 0; j < M - kCAIOlen + 1; ++j) {
              for (int k = 0; k < kCAIOlen && spacetime[t][i+k][j+k] == s[k]; ++k) {
                if (k == kCAIOlen - 1) {
                  score += 10000;
                }
              }
      ...
      {
        static const int kCADiffLoss = 0;//15;
        for (int j = 0; j < kCAIOlen; ++j) {
      ...
      {
        static const int BA = 0;//2 * (H * N + N * M + M * H);
        static const int kCABorderAreaLoss = 0;//3000;
      ...
      {
        static const int kCALastAreaLoss = 0;//10;
        int area = 0;
        for (int i = 0; i < N; ++i) {
          for (int j = 0; j < M; ++j) {
      ...
      score -= ((double)cnt_nonzero_last_layer / A - kCAFracLastArea) * kCAFracLastArea
          ...
          * kCALastAreaLoss;*/
      {
        static const double kCARotatedSubstringsBonus = 0;//20;
        static const double kCARotatedTrashLoss = 0;//9;
      ...
      {
        static const double kCAGenesLoss = 0;//0.1;
...
int main() {
  ...
  for (int T = 0;; ++T) {
    ...
    uint8_t spacetime[H][N][M];
      ...
      F[kCAIOi0][kCAIOj0-1] = 4;
    generation[0].run(F, spacetime, true);
