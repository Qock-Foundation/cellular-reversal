...
def show_tensor(tensor, filename):
  ...
  plt.imshow((arr - arr.min()) / (arr.max() - arr.min()))
...
numbers = np.random.randn(cnt, 3, r)
input_images[:, :, n // 2 - 1, n // 2 - r:n // 2 + r] = 1
input_images[:, :, n // 2, n // 2 - r:n // 2 + r:2] = -1
input_images[:, :, n // 2, n // 2 - r + 1:n // 2 + r:2] = torch.tensor(numbers)
output_images[:, :, n // 2 - r:n // 2 + r, n // 2 - 1] = 1
output_images[:, :, n // 2 - r + 1:n // 2 + r:2, n // 2] = -1
output_images[:, :, n // 2 - r:n // 2 + r:2, n // 2] = torch.tensor(numbers)
