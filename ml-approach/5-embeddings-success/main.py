...
class NN(nn.Module):
  ...
  def forward(self, words, io_filenames=None, o_filename=None):
    ...
    word_jl, word_jr = (n - fontsize) // 2, (n + fontsize) // 2
    word_kl, word_kr = fontsize, n - fontsize
    word_shape = 3, word_ir - word_il, word_jr - word_jl, word_kr - word_kl
    fields[:, :, word_il:word_ir, word_jl - fontsize:word_jr - fontsize, word_kl:word_kr] \
    ...
    target[:, :, word_il:word_ir, word_jl + fontsize:word_jr + fontsize, word_kl:word_kr] \
    ...
    return fields[:, :, word_il:word_ir, word_jl + fontsize:word_jr + fontsize, word_kl:word_kr], \
           target[:, :, word_il:word_ir, word_jl + fontsize:word_jr + fontsize, word_kl:word_kr], letters_loss
